        ...
        async for message in websocket:
            try:
                ...
                if mtype == "hello":
                    ...
                    # ── نسخ للحافظة بـ pyperclip (يدعم Unicode + العربية كاملاً) ──
                    clipboard_ok = False
                    try:
                        import pyperclip
                        pyperclip.copy(text)
                        clipboard_ok = True
                        logger.debug("✅ تم نسخ النص بـ pyperclip")
                    except Exception as clip_err:
                        logger.warning(f"pyperclip فشل: {clip_err} — محاولة tkinter")
                        try:
                            ...
                            logger.error(f"فشل نسخ النص تماماً: {tk_err}")

                    # ── رد فوري قبل أي عمل ثانوي ──
                    ...
                    app.add_to_history(f"نص: {preview}", "", device_name)
                    notif_msg = t("copied") if clipboard_ok else f"نص وارد: {preview}"
                    show_notification(APP_NAME, notif_msg)
                    try:
                        import winsound
